...
def port_type_drpdwn = moi_ref.Source_Power_Port_Type_Drpdwn_Val
...
for (def port_type : port_type_xpath) {
    if (port_type.key == port_type_drpdwn) {
        'To select secondary detection value'
        WebUI.click(findTestObject('Object Repository/Python_Aggregation_Moi_Objects/Config_Source_Power_Tests_Moi/dropdown/button_Port_Type'))

        WebUI.delay(GlobalVariable.short_delay)

        WebElement port_type_ele = driver.findElement(By.xpath(('(//div[@aria-labelledby="tcSorcePowerPortTypeComboBox"]/a)[' + 
                port_type.value) + ']'))

        JavascriptExecutor executor = ((driver) as JavascriptExecutor)

        executor.executeScript('arguments[0].click();', port_type_ele)
    }
}
}
catch (Exception e) {
