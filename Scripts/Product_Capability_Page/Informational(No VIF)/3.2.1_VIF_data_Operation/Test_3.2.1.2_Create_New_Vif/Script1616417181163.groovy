...
WebUI.comment('TestcaseName:: '+'Test_3.2.1.2_Create_New_Vif'+'\n')
...
for (String pc_create_vif_dut : pc_create_vif_dut_list) {
    ...
    if (set_default_yes_value_sts) {
        'STEP 5:To select Yes option'
        WebUI.comment('Set default Value status YES::' + set_default_yes_value_sts+'\n')
    } else {
   ...
   WebUI.callTestCase(findTestCase('xml_verifier/Test_Generate_Vif_Xml_File_Verifier'), [('key1') : pc_create_vif_dut], FailureHandling.STOP_ON_FAILURE)
}

WebUI.comment('********* Test_3.2.1.2_Create_New_Vif Execution Ended *********'+'\n')
