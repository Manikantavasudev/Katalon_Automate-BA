...
try
{
	...
	//WebUI.sendKeys(findTestObject('Object Repository/Send_keys_function/Load_xml_file'),vif_file_path)
	WebUI.callTestCase(findTestCase('Load_Xml_File_RobotFramework'), [('key1') : vif_file_path], FailureHandling.STOP_ON_FAILURE)

	println(WebUI.getText(findTestObject('Object Repository/Send_keys_function/Load_xml_file')) != "Load XML VIF File")
	if( WebUI.getText(findTestObject('Object Repository/Send_keys_function/Load_xml_file')) != "Load XML VIF File")
	{
		...
		if (selected_dut_type == 'Cable') {
			WebUI.callTestCase(findTestCase('Test Cases/Common_Data_Reader/Comparison_Tool_Moi_Data_Reader/Moi_Selection/Moi_Selection_CableDut'),
...
{
	WebUI.comment("enable Admin mode")
}