...
if( WebUI.getText(findTestObject('Object Repository/Send_keys_function/Load_xml_file')) != "Load XML VIF File")
{
	...
	{
		CustomKeywords.'grl_usb_pd_c2_Testcase.Specific.forDRPMOISelection'()
	}
	else if(selected_dut_type =="Consumer Only" )
	{
		CustomKeywords.'grl_usb_pd_c2_Testcase.Specific.forConsumerOnly'()
	}
	else if(selected_dut_type =="Cable")
	{
		CustomKeywords.'grl_usb_pd_c2_Testcase.Specific.forCable'()
	}
	else if(selected_dut_type== "Provider Only")
	{
		CustomKeywords.'grl_usb_pd_c2_Testcase.Specific.ProviderOnly'()
	}
	else
	{
	...
	"To rerun failed & incomplete testcase	"
	try
	{
		'To click Testconfig page'
		WebUI.click(findTestObject('Object Repository/AllTabs_Entry_Page/Page_GRL - USB PD C2/span_Test Config'))

		WebUI.click(findTestObject('Object Repository/Rerun/Failed_Testcase'))

		WebUI.click(findTestObject('Object Repository/Rerun/Incomplete'))
	...
	{
		println("unable to select rerun")
	}
	'To read Test Execution status'
	int j=0
	while(j<=2)
