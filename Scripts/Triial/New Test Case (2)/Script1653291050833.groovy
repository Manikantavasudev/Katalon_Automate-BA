...
vif_file = user_config_vif_file_path

avoid_simple = [':']

WebUI.comment('HERE is the Validation data from  VIF file to UI' + '\n')

verifier(cellvalue, vif_file, python_path, Installation_path) //verifier help to validate the data of vif file


def verifier(def cellvalue, def vif_file, def python_path, def Installation_path) {
    def result = false
    ...
    def task = [python_path, Installation_path + '\\Test Cases\\Python_files\\Xml_Value_Reader_Updated.py', "$vif_file"].execute()

    def json_value = new JsonSlurper().setType(JsonParserType.LAX).parseText(task.text)

    println('json value ' + json_value)

    for (def file_data : json_value) {
        def temp_val
            ...
            if ((all_col_data[1]) != '') {
                ...
                if (file_data.value != '') {
                    if (file_data.key == (all_col_data[0])) {
                        if (((file_data.key.contains('Src PDO Max Current') || file_data.key.contains('Src PDO Min Voltage')) || 
                        file_data.key.contains('Src PDO Max Voltage')) || file_data.key.contains('Src PDO Voltage')||file_data.key.contains('Src PDO Supply Type')||file_data.key.contains('Snk PDO Min Voltage')||file_data.key.contains('Snk PDO Max Voltage')||file_data.key.contains('Snk PDO Op Current')||file_data.key.contains('Snk PDO Supply Type')) {
...
def print_report(def param, def data, def result) {
