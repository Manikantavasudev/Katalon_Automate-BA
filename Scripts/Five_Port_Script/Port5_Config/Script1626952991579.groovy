...
File port5_config_obj = new File('Five_Port_Data_Config\\port5_config.json')

Map port5_info = new JsonSlurper().parseFile(port5_config_obj, 'UTF-8')
...
def vif_file_path = port5_info.Vif_File_Path

def golden_reports_path = port5_info.Golden_Reports_Path

List moi_list_checkbox = port5_info.MOI_List

'gerneral values'
def port5_val = '5'

def project_name = 'Port5'
...
CustomKeywords.'grl_usb_pd_c2_Testcase.Five_Port.configureSelectPort'(port5_val)

'To Enable port in options page'
CustomKeywords.'grl_usb_pd_c2_Testcase.Five_Port.enablePort'(port5_val, findTestObject('Object Repository/Five_Port_Objects/Options/Port_Info/div_Enable_Port5_Toggle'))
...
if (selected_dut_type == 'Cable') {
	WebUI.callTestCase(findTestCase('Common_Data_Reader/Five_Port_Moi_Data_Reader/Moi_Selection/Moi_Selection_CableDut'),
			[('key_json') : 'port5_config.json'], FailureHandling.STOP_ON_FAILURE)
} else {
	WebUI.callTestCase(findTestCase('Common_Data_Reader/Five_Port_Moi_Data_Reader/Moi_Selection/Moi_Selection_OtherDut'),
			[('key_dut') : selected_dut_type, ('key_json') : 'port5_config.json'], FailureHandling.STOP_ON_FAILURE)
}
...
CustomKeywords.'grl_usb_pd_c2_Testcase.Five_Port.disablePort'(port5_val, findTestObject('Object Repository/Five_Port_Objects/Options/Port_Info/div_Enable_Port5_Toggle'))
...
while (json_config_itr_cnt < json_config_itr_limit) {
	...
	if (file_location.contains('Port5')) {
		break
	}
	WebUI.delay(GlobalVariable.short_delay)
...
String moi_config_json_path = Installation_path + '\\Five_Port_Data_Config\\port5_config.json'
...
while (html_config_itr_cnt < html_config_itr_limit) {
	...
	try {
		...
		txt_file_path = new File( Installation_path + '/'+'Five_Port_Data_Config/PyFiles/temp_port'+'/' + 'Port5.txt')
...
def task_alter_report = [python_path, "$alter_report_struct", summary_report_folder, 'Port5'].execute()
...
def task_integrate_report = [python_path, "$integrate_xls_summary ", 'Port5'].execute()
...
while (integrate_sum_itr_cnt < integrate_sum_itr_limit) {
	...
	try {
		...
		summary_file_path = new File(compare_report_path + '/'+summary_report_folder+'/' + 'Summary-Port5.xlsx')
